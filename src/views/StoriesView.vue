...
const filterMap = {
  all: () => true, original: s => ['1','3','4','7'].includes(s.id), fanwork: s => ['2','5','6','8'].includes(s.id),
  romance: s => ['1','2','4'].includes(s.id), fantasy: s => ['1','5','8'].includes(s.id), modern: s => ['2','6','7'].includes(s.id),
  historical: s => ['3','8'].includes(s.id), healing: s => ['2'].includes(s.id),
}

const filteredStories = computed(() => {
  let stories = allStories.filter(filterMap[activeFilter.value] || filterMap.all)
  if (activeMood.value !== 'all') stories = stories.filter(s => s.moods.includes(activeMood.value))
  return stories
})

const countryTabs = [
  { key: 'cn', label: 'gameCatCN' }, { key: 'jp', label: 'gameCatJP' }, { key: 'kr', label: 'gameCatKR' }, { key: 'en', label: 'gameCatEN' },
]
...
.pg-s{font-size:13px;color:var(--ts);letter-spacing:.5px;max-width:420px;margin:0 auto;line-height:1.7}
.filters{display:flex;flex-wrap:wrap;gap:6px;justify-content:center;margin-bottom:14px}
.filter-btn{padding:5px 14px;border:1px solid var(--bd);background:transparent;font-size:12px;color:var(--ts);cursor:pointer;font-family:inherit;transition:all var(--tr)}.filter-btn:hover{border-color:var(--ts);color:var(--tx)}.filter-btn.active{border-color:var(--accent);color:var(--accent)}
.mood-filters{display:flex;flex-wrap:wrap;gap:5px;justify-content:center;margin-bottom:32px;align-items:center}
.mood-label{font-size:10px;color:var(--tm);letter-spacing:2px;text-transform:uppercase;margin-right:6px}
.mood-btn{padding:3px 12px;background:var(--bg2);font-size:11px;color:var(--ts);cursor:pointer;font-family:inherit;transition:all var(--tr)}.mood-btn:hover{background:var(--t);color:var(--tx)}.mood-btn.active{background:var(--accent);color:var(--cw)}
.sc-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:var(--bd)}
.sc{background:var(--bg);padding:24px 20px 20px;transition:all var(--tr);display:flex;flex-direction:column}.sc:hover{background:var(--cw)}
.sc-im{height:120px;display:flex;align-items:flex-start;padding:12px;margin:-24px -20px 16px;position:relative}
.im-1{background:linear-gradient(135deg,var(--bg2),var(--t))}.im-2{background:linear-gradient(135deg,var(--bg2),var(--pp))}.im-3{background:linear-gradient(135deg,var(--bg2),var(--p))}
.sc-tag{font-size:9px;font-weight:500;letter-spacing:1px;color:var(--ts);background:rgba(255,255,255,.8);padding:3px 10px}
.sc-bd{flex:1;display:flex;flex-direction:column}
.sc-t{font-family:'Noto Serif SC','Playfair Display',serif;font-size:14px;font-weight:600;color:var(--tx);letter-spacing:.8px;margin-bottom:3px}
.en .sc-t{font-family:'Playfair Display','Noto Serif SC',serif}
.sc-au{font-size:11px;color:var(--tm);letter-spacing:.3px;margin-bottom:8px}
.sc-d{font-size:12px;color:var(--ts);line-height:1.6;letter-spacing:.2px;margin-bottom:10px;flex:1}
.sc-mood{display:flex;flex-wrap:wrap;gap:3px;margin-bottom:8px}
.sc-mood-tag{font-size:9px;padding:2px 7px;background:var(--bg2);color:var(--tm)}
.sc-meta{display:flex;justify-content:space-between;align-items:center;margin-top:auto;padding-top:8px;border-top:1px solid var(--bd)}
.sc-cat{font-size:10px;color:var(--tm);letter-spacing:.5px}.sc-time{font-size:10px;color:var(--tm)}
.game-tabs{display:flex;gap:8px;justify-content:center;margin-bottom:28px;flex-wrap:wrap}
.game-tab{padding:5px 18px;border:1px solid var(--bd);background:transparent;font-size:12px;color:var(--ts);cursor:pointer;font-family:inherit;transition:all var(--tr)}.game-tab:hover{border-color:var(--ts);color:var(--tx)}.game-tab.active{border-color:var(--accent);color:var(--accent)}
...
.game-card__img{height:180px;overflow:hidden;background:var(--bg2);display:flex;align-items:center;justify-content:center}
...
.game-card__placeholder{font-size:32px;font-weight:700;color:rgba(255,255,255,.15)}
.game-card__bd{padding:16px 20px 20px;flex:1;display:flex;flex-direction:column}
.game-card__t{font-family:'Noto Serif SC','Playfair Display',serif;font-size:14px;font-weight:600;color:var(--tx);letter-spacing:.8px;margin-bottom:2px}
...
.game-card__d{font-size:12px;color:var(--ts);line-height:1.6;letter-spacing:.3px;flex:1}
@media(max-width:968px){.sc-grid{grid-template-columns:repeat(2,1fr)}.game-grid{grid-template-columns:repeat(2,1fr)}.pg:first-of-type{padding-top:100px}}
@media(max-width:640px){.sc-grid{grid-template-columns:1fr}.game-grid{grid-template-columns:1fr;max-width:360px;margin:0 auto}.pg{padding:32px 16px 60px}.game-card__img{height:160px}}
</style>
