<template>
  <div class="pg">
    ...
    <div class="pg-bc reveal">
      ...
      <span class="pg-bcc">{{ t('navAbout') }}</span>
    </div>

    <div class="pg-h reveal">
      <h1 class="pg-t">{{ t('aboutTitle') }}</h1>
    </div>

    <div class="about-body">
      <p class="about-p reveal">{{ t('aboutDesc1') }}</p>
      <p class="about-p reveal reveal-delay-1">{{ t('aboutDesc2') }}</p>
      <p class="about-p reveal reveal-delay-2">{{ t('aboutDesc3') }}</p>
    </div>
  </div>

  <div class="pg">
    <div class="pg-h reveal">
      <h2 class="pg-t" style="font-size:clamp(20px,2.5vw,26px)">{{ t('values') || 'Values' }}</h2>
    </div>

    <div class="values-grid">
      <div v-for="(v, i) in values" :key="i" class="value-card reveal-s">
        <div class="value-icon" :class="'vicon-'+i"></div>
        <h3 class="value-t">{{ t(v.titleKey) }}</h3>
        <p class="value-d">{{ t(v.descKey) }}</p>
      </div>
    </div>
  </div>
</template>
...
const values = [
  { titleKey: 'aboutMission1Title', descKey: 'aboutMission1Desc' },
  { titleKey: 'aboutMission2Title', descKey: 'aboutMission2Desc' },
  { titleKey: 'aboutMission3Title', descKey: 'aboutMission3Desc' },
]
...
<style scoped>
.pg{max-width:720px;margin:0 auto;padding:48px 24px 100px}
...
.pg-t{font-family:'Noto Serif SC','Playfair Display',serif;font-size:clamp(24px,3vw,32px);font-weight:600;color:var(--tx);letter-spacing:2px;margin-bottom:14px}
.en .pg-t{font-family:'Playfair Display','Noto Serif SC',serif}

.about-body{max-width:600px;margin:0 auto}
.about-p{font-size:14px;color:var(--ts);line-height:2;letter-spacing:.5px;margin-bottom:28px}
.about-p:last-child{margin-bottom:0}

.values-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--bd)}
.value-card{background:var(--bg);padding:48px 32px;transition:all var(--tr);text-align:center}
.value-card:hover{background:var(--cw)}
.value-icon{width:40px;height:40px;margin:0 auto 20px;position:relative;background:var(--t)}
.value-icon::after{content:'';position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:18px;height:18px;background:var(--ts);mask-size:contain;-webkit-mask-size:contain;mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat}
...
.value-t{font-family:'Noto Serif SC','Playfair Display',serif;font-size:16px;font-weight:600;color:var(--tx);letter-spacing:.8px;margin-bottom:10px}
.value-d{font-size:13px;color:var(--ts);line-height:1.7;letter-spacing:.3px;max-width:280px;margin:0 auto}

@media(max-width:968px){
  .pg:first-of-type{padding-top:110px}
  .values-grid{grid-template-columns:1fr}
  .value-card{padding:36px 28px}
  .value-d{max-width:100%}
}
